/*
 * drivers/cpufreq/cpufreq_dark_stealth.c
 *
 * Copyright (C) 2010 Google, Inc.
 * Copyright (C) 2017 Pranay Narang <pranaynarang@gmail.com>
 *
 * Dark_Stealth Version 1.0
 *
...
#include <trace/events/cpufreq_dark_stealth.h>

static int active_count;

static bool ds_locked = false;

struct cpufreq_dark_stealth_cpuinfo {
...
static DEFINE_PER_CPU(struct cpufreq_dark_stealth_cpuinfo, cpuinfo);
...
static int cpufreq_governor_dark_stealth(struct cpufreq_policy *policy,
		unsigned int event);

#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_DARK_STEALTH
static
#endif
struct cpufreq_governor cpufreq_gov_dark_stealth = {
	.name = "dark_stealth",
	.governor = cpufreq_governor_dark_stealth,
	.max_transition_latency = 10000000,
	.owner = THIS_MODULE,
};

static void cpufreq_dark_stealth_timer_resched(
	struct cpufreq_dark_stealth_cpuinfo *pcpu)
{
 ...
 * function.
 */
static void cpufreq_dark_stealth_timer_start(int cpu, int time_override)
{
	struct cpufreq_dark_stealth_cpuinfo *pcpu = &per_cpu(cpuinfo, cpu);
	unsigned long flags;
	unsigned long expires;
	if (time_override)
...
static unsigned int choose_freq(
	struct cpufreq_dark_stealth_cpuinfo *pcpu, unsigned int loadadjfreq)
{
...
static void cpufreq_dark_stealth_timer(unsigned long data)
{
	u64 now;
	unsigned int delta_time;
	u64 cputime_speedadj;
	...
	struct cpufreq_dark_stealth_cpuinfo *picpu;
	...
	if (cpu_load <= MAX_LOCAL_LOAD &&
		...
		trace_cpufreq_dark_stealth_notyet(
	...
	if (pcpu->target_freq == new_freq &&
		...
		trace_cpufreq_dark_stealth_already(
			data, cpu_load, pcpu->target_freq,
			pcpu->policy->cur, new_freq);
		goto rearm_if_notmax;
	}

	trace_cpufreq_dark_stealth_target(data, cpu_load, pcpu->target_freq,
					 pcpu->policy->cur, new_freq);
...
rearm:
	if (!timer_pending(&pcpu->cpu_timer))
		cpufreq_dark_stealth_timer_resched(pcpu);
...
static void cpufreq_dark_stealth_idle_start(void)
{
	struct cpufreq_dark_stealth_cpuinfo *pcpu =
...
static void cpufreq_dark_stealth_idle_end(void)
{
	struct cpufreq_dark_stealth_cpuinfo *pcpu =
		&per_cpu(cpuinfo, smp_processor_id());
	...
	if (!timer_pending(&pcpu->cpu_timer)) {
		...
		cpufreq_dark_stealth_timer(smp_processor_id());
	}

	up_read(&pcpu->enable_sem);
}

static int cpufreq_dark_stealth_speedchange_task(void *data)
{
	unsigned int cpu;
	cpumask_t tmp_mask;
	unsigned long flags;
	...
	while (1) {
		...
		for_each_cpu(cpu, &tmp_mask) {
			...
			struct cpufreq_dark_stealth_cpuinfo *pjcpu;
			...
			trace_cpufreq_dark_stealth_setspeed(cpu,
						     pcpu->target_freq,
						     pcpu->policy->cur);
...
static void cpufreq_dark_stealth_boost(void)
{
	int i;
	int anyboost = 0;
	unsigned long flags;
...
static int cpufreq_dark_stealth_notifier(
...
static struct notifier_block cpufreq_notifier_block = {
	.notifier_call = cpufreq_dark_stealth_notifier,
};
...
static ssize_t show_ds_locked(struct kobject *kobj,
			struct attribute *attr, char *buf)
{
	if (ds_locked)
...
static ssize_t store_ds_locked(
	struct kobject *kobj, struct attribute *attr, const char *buf,
	size_t count)
{
	int ret;
...
static struct global_attr ds_locked_attr = __ATTR(ds_locked, 0644,
		show_ds_locked, store_ds_locked);
			   ...
			   const char *buf, size_t count)
{
	...
	if (boost_val) {
		trace_cpufreq_dark_stealth_boost("on");
		if (!boosted)
			cpufreq_dark_stealth_boost();
	} else {
		trace_cpufreq_dark_stealth_unboost("off");
	}
				...
				const char *buf, size_t count)
{
	...
	trace_cpufreq_dark_stealth_boost("pulse");
	if (!boosted)
		cpufreq_dark_stealth_boost();
	return count;
}
...
static struct attribute *dark_stealth_attributes[] = {
	&ds_locked_attr.attr,
...
static struct attribute_group dark_stealth_attr_group = {
	.attrs = dark_stealth_attributes,
	.name = "dark_stealth",
};

static int cpufreq_dark_stealth_idle_notifier(struct notifier_block *nb,
					     unsigned long val,
					     void *data)
{
	switch (val) {
	case IDLE_START:
		cpufreq_dark_stealth_idle_start();
		break;
	case IDLE_END:
		cpufreq_dark_stealth_idle_end();
		break;
	}
...
static struct notifier_block cpufreq_dark_stealth_idle_nb = {
	.notifier_call = cpufreq_dark_stealth_idle_notifier,
};
		...
		unsigned int event)
{
	...
	case CPUFREQ_GOV_START:
		...
		for_each_cpu(j, policy->cpus) {
			...
			cpufreq_dark_stealth_timer_start(j, 0);
		...
		rc = sysfs_create_group(get_governor_parent_kobj(policy),
				&dark_stealth_attr_group);
		if (rc) {
			mutex_unlock(&gov_lock);
			return rc;
		}

		idle_notifier_register(&cpufreq_dark_stealth_idle_nb);
	...
	case CPUFREQ_GOV_STOP:
		...
		cpufreq_unregister_notifier(
			&cpufreq_notifier_block, CPUFREQ_TRANSITION_NOTIFIER);
		idle_notifier_unregister(&cpufreq_dark_stealth_idle_nb);
	...
	case CPUFREQ_GOV_LIMITS:
		...
		for_each_cpu(j, policy->cpus) {
			...
			if (pcpu->nr_timer_resched) {
				...
				cpufreq_dark_stealth_timer_start(j, expire_time);
				pcpu->nr_timer_resched++;
			}
...
static void cpufreq_dark_stealth_nop_timer(unsigned long data)
{
}

static int __init cpufreq_dark_stealth_init(void)
{
	unsigned int i;
	struct cpufreq_dark_stealth_cpuinfo *pcpu;
	...
	for_each_possible_cpu(i) {
		...
		pcpu->cpu_timer.function = cpufreq_dark_stealth_timer;
		pcpu->cpu_timer.data = i;
		init_timer(&pcpu->cpu_slack_timer);
		pcpu->cpu_slack_timer.function = cpufreq_dark_stealth_nop_timer;
	...
	mutex_init(&gov_lock);
	speedchange_task =
		kthread_create(cpufreq_dark_stealth_speedchange_task, NULL,
			       "cfdark_stealth");
	...
	return cpufreq_register_governor(&cpufreq_gov_dark_stealth);
}

#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_DARK_STEALTH
fs_initcall(cpufreq_dark_stealth_init);
#else
module_init(cpufreq_dark_stealth_init);
#endif

static void __exit cpufreq_dark_stealth_exit(void)
{
	cpufreq_unregister_governor(&cpufreq_gov_dark_stealth);
	kthread_stop(speedchange_task);
	put_task_struct(speedchange_task);
}

module_exit(cpufreq_dark_stealth_exit);

MODULE_AUTHOR("Pranay Narang <pranaynarang@gmail.com>");
MODULE_DESCRIPTION("'cpufreq_dark_stealth'");
MODULE_LICENSE("GPL");
